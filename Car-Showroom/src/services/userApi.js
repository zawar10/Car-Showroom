...
export const getUsersApi = () => request(() => userApi.get('/users/user'), () => demoUsers());
export const createUserApi = (payload) => request(() => userApi.post('/users/user', payload), () => {
  const users = demoUsers();
  const user = { ...payload, id: generateId('USR'), createdAt: new Date().toISOString() };
  setData(STORAGE_KEYS.users, [user, ...users]);
  return user;
});
export const updateUserApi = (id, payload) => request(() => userApi.put('/users/user', { id, ...payload }), () => {
  const users = demoUsers();
  const user = { ...users.find(item => item.id === id), ...payload, id };
  setData(STORAGE_KEYS.users, users.map(item => item.id === id ? user : item));
  return user;
});
export const deleteUserApi = (id) => request(() => userApi.delete(`/users/user/${id}`), () => {
