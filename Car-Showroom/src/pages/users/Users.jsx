...
import { clearUsersError, clearUsersSuccess } from '../../features/users/usersSlice'
import { selectUsersList, selectUsersError, selectUsersLoading, selectUsersSuccess } from '../../features/users/usersSelectors'
...
export default function Users() {
  ...
  useEffect(() => { if (!success) return undefined; const timer = setTimeout(() => dispatch(clearUsersSuccess()), 2500); return () => clearTimeout(timer) }, [dispatch, success])
  ...
  const editUser = user => { dispatch(clearUsersError()); setEditingUser(user); setFormOpen(true) }
  return <>
    ...
    {error && <div className="error user-alert" role="alert">{error}<button onClick={() => dispatch(clearUsersError())}>Dismiss</button></div>}
